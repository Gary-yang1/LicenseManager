...
func ValidAppLic(appInfoFile, key string) (res bool, AuthorizedName string, LimitedTime string, err error) {
 file, err := os.OpenFile(appInfoFile, os.O_RDONLY, 0777)
 if err != nil {
 ...
 if err := json.Unmarshal([]byte(tmpText), &conf); err == nil {
  ...
  id, err := machineid.ID()
  if err != nil {
   return false, conf.AuthorizedName, conf.LimitedTime, errors.New("获取本机ID失败")
  }
  if conf.ObjUUID != id {
   return false, conf.AuthorizedName, conf.LimitedTime, errors.New("授权文件不适用于此设备")
  }
  ...
  if limitedTime != "" {
   ...
   if licDate < currentDate {
    ...
    return false, conf.AuthorizedName, conf.LimitedTime, errors.New(errInfo)
   }
  }

 }
 return true, conf.AuthorizedName, conf.LimitedTime, nil
}
